...
Syntalume follows the VS Code display language automatically. No Syntalume setting
...
Theme names such as `Syntalume Botanica`, setting IDs such as
...
VS Code owns that language choice; Syntalume reads it through the standard
extension localization API.

If a translated label is missing or unclear, run **Syntalume: Request Language
Coverage**. The command opens the public support form only after you choose to
use it; Syntalume never sends language or diagnostic data automatically.
...
safe fallback for locales Syntalume does not yet ship.
