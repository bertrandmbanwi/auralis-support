...
Versioned, generated Auralis themes for syntax renderers, documentation, and community ports. The files are derived from the exact same source as the VS Code, JetBrains, and terminal releases.
...
Every release includes `dist/index.json`, which follows the public Auralis palette v1 schema and carries editor, accent, and complete ANSI terminal metadata for all nine variants.
...
The packaged `LICENSE` includes the Auralis palette and community-port
exception. It permits syntax/documentation use and attributed Auralis ports;
